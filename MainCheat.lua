...
local cameraLockEnabled = false  -- Variable to track if the camera lock is enabled
...
-- Function to enable or disable the camera lock
local function toggleCameraLock(enabled)
    cameraLockEnabled = enabled
    if not cameraLockEnabled then
        holdingF = false  -- Stop holding F if camera lock is disabled
    end
end
...
-- Toggle for Camera Lock (Using a GUI Toggle)
TsbTab:AddToggle({
...
Players.LocalPlayer.CharacterAdded:Connect(function()
    -- Reset camera lock state when respawned
